...
Ranking scaffolds is a key step in prioritizing hits in a
...
is an single, optimal method).  Some features that are maybe
considered include the size and complexity of a scaffold, which is
usually application dependent (as noted by Ertl et al
\cite{Ertl:2014eu}). Complexity can be important as highly complex
scaffolds may not be easily purchasable or synthesizable. Scaffolds
can also be characterized in terms of promiscuity - does a scaffold
contain many active compounds within an assay or across multiple
assays? Finally, a reasonable expectation is that members of a
scaffold exhibit an SAR and scaffolds for which no SAR is observed
could be used to deprioritize compounds containing that scaffold.
...
overlapping scaffolds. We observe that this method, applied to the
kinase XXX dataset can be a powerful tool to identify promising
hits. While a simple procedure, the use of the Spotfire workflow
allows interactive drill down from aggregate properties to individual
compounds. In essence, the workflow described here enables decisions
on individual compounds, but using the aggregated data as a
filter. Another advantage of the workflow described here is that it
supports the inclusion of ``virtual datasets'', where there is no
measured activity, as highlighted in the data fusion use
case.  Inclusion of such datasets can be useful as they provide an
opportunity to directly higlight untested regions of chemical
space. When multiple datasets, with measured activities, are included
in the data fusion, it provides the opportunity to increase confidence
in noisy data by merging data for scaffolds across the datasets.
...
proportion ($CP$), fragment efficiency ($FragEff$) and PIU places such
comparisons on a sound statistical basis, allowing for an objective
comparison of fragmentation methods for given goal. As mentioned
earlier they also extend the applicability of methods to compare the
output of different partitioning clustering methods such as
\citeauthor{Torres2009}, allowing them to be used for non-overlapping
fuzzy clusters.

We note that the approaches described here assume a set of precomputed
fragments. While fragmentation could be done on the fly, it is
prohibitive for large screening collections. However, given that the
fragmentation needs to be computed once (followed by updates as new
compounds are added to the collection), this is not overly
burdensome. Precomputed compound-scaffold memberships reduce to
fixed-time lookups, using appropriate database indexing methods.
...
highlights the fact that no single fuzzy clustering methods is
optimal, and the most appropriate approach should be selected based on
the types of analyses described here. It is a given that chemical
