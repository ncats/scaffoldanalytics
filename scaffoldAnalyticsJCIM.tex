...
decomposition methods described expect MDL SD-files. We constructed a
Pipeline Pilot workflow to convert CSV to SDF while preserving
non-molecule fields as SDF properties.  Prior to SDF conversion,
activity or property columns that are not to be aggregated at the
scaffold level should be deleted from the CSV file, in order to speed
up the analysis and aggregation.  There are some further quirks to
pre-processing datasets for the NCATS R-group tool that will be
described in the Supplementary Material.\textbf{\textcolor{red}{follow
    through. Did you get this?}}
...
\end{itemize}
The remaining columns are aggregated property columns. Thus for each
